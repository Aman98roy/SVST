...
export function YogaImageSlider() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setExpandedImageIndex((prev) => (prev + 1) % yogaImages.length);
    }, 4000);
  ...
  const goToPrevious = () => {
    setCurrentIndex((prev) => (prev - 1 + yogaImages.length) % yogaImages.length);
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft") {
        setExpandedImageIndex((prev) => (prev - 1 + yogaImages.length) % yogaImages.length);
      }
      if (e.key === "ArrowRight") {
        setExpandedImageIndex((prev) => (prev + 1) % yogaImages.length);
      }
  ...
  return (
    <>
      ...
      >
        ...
        {/* Expand Button - Modern Design */}
        <motion.button
          onClick={() => openExpanded(currentIndex)}
          whileHover={{ scale: 1.1, rotate: 90 }}
          whileTap={{ scale: 0.9 }}
          className="absolute bottom-6 right-6 z-20 p-4 bg-gradient-to-br from-primary-500/80 to-accent-teal/80 hover:from-primary-500 hover:to-accent-teal backdrop-blur-xl rounded-2xl transition-all duration-300 opacity-0 group-hover:opacity-100 shadow-2xl border border-white/30"
        ...
        <div className="absolute top-6 right-6 z-20 px-4 py-2 bg-black/40 backdrop-blur-md rounded-full text-white text-sm font-semibold border border-white/20 opacity-0 group-hover:opacity-100 transition-opacity">
          {currentIndex + 1} / {yogaImages.length}
        </div>
      </motion.div>
          ...
          >
            ...
            >
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <Image
                    src={yogaImages[expandedImageIndex]}
                    alt={`Yoga Day ${expandedImageIndex + 1}`}
                    fill
              ...
              {yogaImages.length > 1 && (
                <>
                  ...
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[200] px-6 py-3 bg-gray-900 rounded-full text-white text-lg font-bold border-4 border-white shadow-[0_0_20px_rgba(0,0,0,0.8)]">
                    {expandedImageIndex + 1} / {yogaImages.length}
                  </div>
                </>
              )}
            </motion.div>
